...
scefile <- sceExtract(seurat_object, exp_slot_name = "counts") 
...
seuratfile <- seuratExtract(sce_object, exp_type = "counts")
...
# Train SCN Top-Pair Random Forest classifier
...
We use the same number of cells per cell type, i.e. balanced data, to train Top-Pair Random Forest classifier. The remaining of the data or the held-out data will be used as validation data to evaluate the performance of the TP-RF classifier.
...
```{r}
#normalize
tmpX = weighted_down(expDat = expTrain, total = 1.5e3, dThresh=0.25)
#log transform and scale
expTrainNorm <- trans_prop(expDat = tmpX, xFact = 1e4)
```
...
```{r, message=FALSE}
#identify the top 10 most differential expressed genes 
cgenes2<-findClassyGenes(expDat = expTrainNorm, sampTab = stTrain, dLevel = "newAnn", topX=10)

#find the top 10 gene-pairs
xpairs<-ptGetTop(expDat = expTrainNorm[cgenes2$cgenes,], cell_labels = cgenes2$grps, topX=25, sliceSize=5000)

#template matching the training data
pdTrain<-query_transform(expDat = expTrainNorm[cgenes2$cgenes, ], genePairs = xpairs)
```
...
```{r, message=FALSE}
rf_tspAll<-sc_makeClassifier(expTrain = pdTrain, genes = xpairs, groups = cgenes2$grps, nRand = 50, ntrees = 1000)
```

## Assess the Top-pair Random Forest classifier
...
```{r}
#template matching the query data
expQtransAll<-query_transform(expTest, xpairs)

#apply heldout data
classRes_val_all<-rf_classPredict(rfObj = rf_tspAll, expQuery = expQtransAll, numRand=50)
...
```{r}
expPBMCtrans<-query_transform(expQueryOrth[cgenes2$cgenes,], xpairs)

crPBMC<-rf_classPredict(rf_tspAll, expPBMCtrans, numRand=50)
```
...
### Visualize the classification score with a global view
```{r, warning=FALSE}
sc_violinClass(sampTab = stQuery,classRes = crPBMC, cellIDCol = "sample_name", dLevel = "description", ncol = 8)
```

### Visualize classification of selected cluster with violin plot
...
```{R}
sc_violinClass(sampTab = stQuery, classRes = crPBMC, cellIDCol = "sample_name", dLevel = "description", ncol = 8, sub_cluster = "B cell")
```
