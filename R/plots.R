...
# return pbs names in order you want them to appear
subcluster_order<-function(aMat, grps){

  ans <- vector()
  for(grp in unique(grps)){
    subs = names(grps[which(grps==grp)])
    cat(grp ," ",length(subs),"\n")
    xdat = aMat[,subs]
    atree<-hclust( utils_myDist(t(xdat)), 'ave')
    ans <- append( ans, subs[atree$order] )
  }
  grps[ans]
}
